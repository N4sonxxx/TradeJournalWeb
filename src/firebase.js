...
import { getFirestore, collection, doc, onSnapshot, addDoc, updateDoc, deleteDoc, setDoc, query, serverTimestamp, setLogLevel } from "https://www.gstatic.com/firebasejs/11.6.1/firebase-firestore.js";
import { getStorage, ref, uploadBytesResumable, getDownloadURL } from "https://www.gstatic.com/firebasejs/11.6.1/firebase-storage.js";

// --- Konfigurasi Firebase ---
// Variabel ini akan digantikan oleh lingkungan production Anda
...
export default function App() {
  ...
  const [dailyJournals, setDailyJournals] = useState(dummyJournals);
  ...
  const addTransaction = (tx) => {
    ...
    const newTx = { id: new Date().getTime().toString(), ...tx, date: new Date(), status, notes: '', tags: [], rating: 0 };
  ...
  const saveTransactionDetails = (updatedTx) => {
    setTransactions(prev => prev.map(tx => tx.id === updatedTx.id ? updatedTx : tx));
    setViewingTrade(null);
  };

  const saveDailyJournal = (dateString, journalData) => {
    setDailyJournals(prev => ({...prev, [dateString]: journalData}));
  };
  ...
  const { trades, dashboardStats, advancedStats } = useMemo(() => {
    const processedTransactions = transactions.map(tx => ({ ...tx, date: tx.date instanceof Date ? tx.date : new Date() }));
